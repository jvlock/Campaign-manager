 ...
 * No participant population table is queried in open development.
 */
export async function loadOccurrenceSources(client: EvaluationClient, campaignId: string, sessionId: string): Promise<OccurrenceSources> {
  ...
  FROM occurrence s JOIN campaigns c ON c.id=s.campaign_id JOIN activities a ON a.id=s.activity_id AND a.campaign_id=c.id`, [campaignId, sessionId]);
...
export async function lockEvaluationSources(client: EvaluationClient) {
  await client.query(`LOCK TABLE campaigns,activities,webinar_sessions,legacy_activities,legacy_campaigns,
    ...
    communication_ctas,communication_landing_pages,landing_page_content_assets,activity_ownership IN SHARE MODE`);
}
...
export function occurrenceAssemblySource(sources: OccurrenceSources, calculationAt: string): AssemblySource {
