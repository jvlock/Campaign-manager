  ...
  try {
    ...
    try {
      ...
      for (const file of files.filter((name) => name >= "0022_")) {
        ...
        const pre0025 = file.startsWith("0025_") ? await priorPayloadSnapshot(prior) : null;
        await query(prior, `BEGIN;\n${await readFile(join(migrationsDir, file), "utf8")}\nCOMMIT;`);
        if (pre0024) {
        ...
        if (pre0025) {
          await assertPriorPayloadUnchanged(prior, pre0025);
          const counts = await query(prior, `SELECT
            (SELECT count(*)::int FROM webinar_synthetic_fixtures) fixtures,
            (SELECT count(*)::int FROM webinar_lifecycle_events) events,
            (SELECT count(*)::int FROM webinar_lifecycle_obligations) obligations`);
          if (Object.values(counts.rows[0]).some(n => n !== 0)) throw new Error("0025 fabricated legacy participant history");
          console.log(JSON.stringify({ type: "webinar-pre0025-upgrade", completePriorChain: 24,
            priorTableCount: Object.keys(pre0025).length, byteEqual: true, noLegacyBackfill: true }));
        }
      }
