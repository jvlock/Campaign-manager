...
async function startLocal() {
  ...
  if (newCluster) {
    ...
    if (JSON.stringify(manifest.migrations) !== JSON.stringify(hashes)) {
      throw new Error("Existing local cluster migration manifest differs; explicit reviewed upgrade required. No reset performed.");
    }
  }
  ...
  try {
    ...
    await verify(client);
    if (newCluster) await writeFile(join(root, "bootstrap-manifest.json"),
...
try {
  if (command === "--help") {
    console.log(`Usage: node lib/db/scripts/open-development-planning.mjs [dev|setup|stop]
...
Restricted transition requires a separate operational database and verified identities; never reuse this development DB.`);
    ...
    if (!["dev", "setup"].includes(command)) throw new Error("Unknown launcher command; use --help");
    ...
    if (mode === "open-development") {
      ...
      await startLocal();
    } else if (command === "setup") throw new Error("Setup requires explicit open-development mode");
