...
async function verify(client) {
  ...
  const hasFixtures = (await client.query("SELECT to_regclass('public.webinar_synthetic_fixtures') IS NOT NULL AS present")).rows[0].present;
  const contamination = await client.query(`SELECT
    ...
    OR ${hasFixtures ? `EXISTS (SELECT 1 FROM webinar_people p WHERE p.is_synthetic IS DISTINCT FROM true
      OR NOT EXISTS (SELECT 1 FROM webinar_synthetic_fixtures f
        WHERE f.person_id=p.id AND f.campaign_id=p.campaign_id AND f.provenance='synthetic-participant-simulator'
          AND p.name=('Synthetic participant ' || f.fixture_key)))`
      : "EXISTS (SELECT 1 FROM webinar_people)"}
    AS contaminated`);
...
async function startLocal() {
  ...
  if (newCluster) {
    if (command === "upgrade-0024" || command === "upgrade-0025") throw new Error("Upgrade requires an existing reviewed private synthetic cluster");
    ...
    if (JSON.stringify(manifest.migrations) !== JSON.stringify(hashes)) {
      ...
      const prior25 = files.filter(file => file < "0025_");
      upgrade = command === "upgrade-0025" && manifest.marker === "synthetic-open-development-v1" &&
        files.length === 25 && prior25.length === 24 &&
        Object.keys(manifest.migrations ?? {}).length === 24 &&
        prior25.every(file => manifest.migrations[file] === hashes[file]);
      if (!upgrade) throw new Error("Existing local cluster migration manifest differs; explicit reviewed upgrade-0025 required. No reset performed.");
    }
  }
  ...
  try {
    ...
    if (upgrade) {
      ...
      await client.query("BEGIN");
      try {
        await client.query(await readFile(join(dbRoot, "migrations/0025_webinar_synthetic_lifecycle.sql"), "utf8"));
        await client.query("COMMIT");
...
try {
  if (command === "--help") {
    console.log(`Usage: node lib/db/scripts/open-development-planning.mjs [dev|setup|stop|upgrade-0025]
...
upgrade-0025: explicit additive upgrade of an exact checksum-matching 0001-0024 private synthetic cluster; no reset.
...
Restricted transition requires a separate operational database and verified identities; never reuse this development DB.`);
    ...
    if (!["dev", "setup", "upgrade-0025"].includes(command)) throw new Error("Unknown launcher command; use --help");
